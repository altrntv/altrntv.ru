...
$song = R::getCursor('SELECT songs.*, DATE_FORMAT(songs.date, "%M %d, %Y") as date, genres.genre, users.username, users.url FROM songs INNER JOIN genres ON genres.id=songs.genre INNER JOIN users ON users.id=songs.id_user WHERE users.url=? AND songs.url_song=?', array($_GET['artist'], $_GET['song']));
$key = $song->getNextItem();

$album = R::findOne('albums', 'album = ?', [$key['album']]);
if (!$album) {
echo '
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>'.$key['song'].' by '.$key['username']; if ($key['feat']) { echo ' feat. '.$key['feat']; } echo '</title>
...
<body>
    <div id="fullpage">';
        ...
        <div class="content">';
        if ($key) {
            R::exec('UPDATE songs SET visits=visits+1 WHERE id=?', [$key['id']]);
            echo '
            <div class="music">
                        ...
                        <div class="player">
                            <div class="player-block">
                                <div class="image" id="app">
                                    <span class="music__image" style="background-image: url(/img/music/'.$key['image_song'].');">
                                ...
                                <div class="info">
                                    ...
                                    <div id="dinfo" class="box" data-id="'.$key['id'].'" data-info="song">
                                        <h1>'.$key['song'].'</h1>
                                        <h3><span>'; echo $lang ? 'Артист' : 'Artist'; echo ':&nbsp;</span><a href="/'.$key['url'].'">'.$key['username'].'</a>'; if($key['feat']) { echo ' <span class="feat">feat.</span> <a href="/'.transform($key['feat']).'">'.$key['feat'].'</a>'; } echo '</h3>
                                    </div>
                                    <div class="bt">
                                        <span class="album"><i class="fas fa-compact-disc"></i>'.$key['album'].'</span>';
                                        $like = R::count('favourite_songs', 'id_song = ?', [$key['id']]);
                                        $user_like = R::count('favourite_songs', 'id_song = ? AND id_user = ?', [$key['id'], $_SESSION['logged_user']->id]);
                            ...
                            if ($key['cca3']) {
                                echo '
                                <div class="license">
                                    <span>'; echo $lang ? 'трек лицензирован по' : 'track licensed under a'; echo ' </span>
                            ...
                            echo '
                            <img id="srcimg" src="'.ROOT.'/img/music/'.$key['image_song'].'">
                        </div>';
                ...
                <div class="music__right">
                    ...
                    $similar = R::getAll('SELECT songs.song, songs.url_song, songs.image_song, users.username, users.url FROM songs INNER JOIN genres ON genres.id=songs.genre INNER JOIN users ON users.id=songs.id_user WHERE genres.genre=? AND songs.song<>? ORDER BY RAND() LIMIT 3', [$key['genre'], $key['song']]);
                    echo '
                    <div class="similar">';
          			if ($similar) {
                        foreach ($similar as $sim) {
                            ...
                            <div class="similar-music">
                                <a href="/'.$sim['url'].'/'.$sim['url_song'].'">
                                    <div class="similar-image">
                                        <span style="background-image: url(/img/music/'.$sim['image_song'].'); width: 100px; height: 100px;"></span>
                                    </div>
                                    <div class="similar-info">
                                        <span>'.$sim['song'].'</span>
                                        <span>'.$sim['username']; echo $sim['feat'] ? ' feat. '.$sim['feat'] : ''; echo '</span>
                                    </div>
                                </a>
                            </div>';
                        }
            ...
            echo '
            <div class="unknown">
                <div class="error"><i class="fas fa-music"></i></div>
                <div>This track was not found.</div>
            </div>';
        }
        echo '
        </div>
    ...
    <script src="'.ROOT.'/js/song.js"></script>
</body>
</html>';
} else {
    header("Location: ".ROOT."/".$_GET['artist']."/album/".$album['url_album']);
}
?>